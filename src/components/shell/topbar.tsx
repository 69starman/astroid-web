...
/**
 * The workspace top bar: org switcher, global command trigger, AI assistant,
 * notifications, and the account menu. Kept quiet so the page content leads.
 */
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-border bg-background/80 px-4 backdrop-blur-xl sm:px-6">
      ...
      <Dropdown
        ...
        trigger={
          <span className="flex items-center gap-2 rounded-button px-2.5 py-1.5 text-sm font-medium transition-colors duration-fast hover:bg-surface-secondary">
            ...
            <span className="max-w-[140px] truncate">{activeOrg?.name ?? 'Astroid'}</span>
      ...
      <button
        ...
        className="hidden items-center gap-2 rounded-button border border-border bg-surface px-3 py-1.5 text-xs text-foreground-muted transition-colors duration-fast hover:border-border-strong hover:text-foreground-secondary sm:flex"
      ...
      <Dropdown
        trigger={
          <span className="ml-1 flex items-center gap-2 rounded-full transition-opacity duration-fast hover:opacity-80">
